...
Работодатель может просматривать резюме только, если у компании есть отклик/приглашение на это резюме для активной вакансии.
...
contact[].value | string или object или null | Значение контакта. Для телефона  – объект из четырех полей `country `, `city`, `number`, `formatted`, для email — строка.
...
contact[].verified | boolean | Является ли телефон подтвержденным (при указании телефона)
...
experience[].company | string или null | Организация
...
experience[].company_url или null | string | Cайт компании
...
hidden_fields | array | Список скрытых полей. Элемент справочника [resume_hidden_fields](dictionaries.md) ([подробнее](#hidden-fields))
...
Работодатель может скачать резюме соискателя только, если у компании есть отклик/приглашение на это резюме для активной вакансии. 
...
## Скрытые поля в резюме

В поле `hidden_fields` содержится список скрытых соискателем полей резюме. Скрытые поля заменяются на `null`. Автору резюме выдаются актуальные данные.
...
* `experience` — заменяет на `null` поля `experience[].company`, `experience[].company_id`, `experience[].company_url`; заменяет значение поля `recommendation` на пустой список
